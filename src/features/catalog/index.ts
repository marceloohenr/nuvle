export { CatalogProvider, useCatalog } from './context/CatalogContext';
export type { CategoryActionResult, ProductDraft } from './context/CatalogContext';
export { default as ProductCard } from './components/ProductCard';
export { default as ProductModal } from './components/ProductModal';
export { default as SearchModal } from './components/SearchModal';
export { products } from './data/products';
export type { Product, ProductCategory, ProductCategoryMeta, SizeGuideRow } from './types/product';
